...
function fun() {
	...
	timeout=10
	curl_url_ok=`timeout 3 curl -I -k -u xxxx:xxxx https://xxxx |grep HTTP |awk '{print $2}'`
	curl_url_timeout=`timeout ${timeout} curl -I -k -u xxxx:xxxx https://xxxx |grep HTTP |wc -l`
      }
...
function check() {
           if [ ${curl_url_ok} -eq 200 ] && [ ${curl_url_timeout} -eq 1 ]; then
           	echo "https://xxxxxx ELKstak web recover" | mail -s "ELK系统页面已恢复" ${mail_user}
		...
		do
			sleep 1
				if [ ${curl_url_ok} -ne 200 ] || [ ${curl_url_timeout} -eq 0 ]; then
					echo "https://xxxxxx ELKstak web is down" | mail -s "ELK系统页面无法访问" ${mail_user}
					errors
					while :
					do
					fun
