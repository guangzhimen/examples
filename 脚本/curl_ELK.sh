...
function errors() {
	   if [ ${curl_url_ok} != "200" ]; then
		echo `date +'%Y%m%d-%T'` 'elk web is error' >> /root/monitor/monitor-`date +'%y%m%d'`.log
	   fi
	   if [ ${curl_url_timeout} -eq 0 ]; then
		echo `date +'%Y%m%d-%T'` 'elk web is timeout > 10s' >> /root/monitor/monitor-`date +'%y%m%d'`.log
	   fi
}
...
function check() {
           if [ ${curl_url_ok} -eq 200 ] && [ ${curl_url_timeout} -eq 1 ]; then
           	...
           	echo `date +'%Y%m%d-%T'` 'elk web [recover]' >> /root/monitor/monitor-`date +'%y%m%d'`.log
                break
           fi
        }
